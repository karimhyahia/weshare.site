...
import { ExternalLink, MapPin, Star, ArrowRight, Layout, Phone, Smartphone, MessageCircle, Mail } from 'lucide-react';
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 py-12 space-y-24">
          ...
          <div className="flex-1 space-y-8 text-center md:text-left animate-fade-in">
             ...
             <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                ...
                {data.links.slice(0, 2).map(link => (
                    <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 transition-all hover:scale-105">
        ...
        <footer className="border-t border-slate-100 pt-12 pb-8 text-center">
             <div className="flex justify-center gap-6 mb-8">
                {data.links.map(link => (
                    <a key={link.id} href={link.url} className="text-slate-400 hover:text-slate-900 transition-colors">
                        {getIcon(link.icon)}
                    </a>
                ))}
             </div>
