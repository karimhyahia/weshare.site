...
// --- Image Cropper Component (Kept same logic) ---
...
export const EditorPanel: React.FC<EditorPanelProps> = ({ data, onChange }) => {
  ...
  const handleProjectImageUpload = (id: string, event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => { if (typeof reader.result === 'string') updateProject(id, 'imageUrl', reader.result); };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleLogoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => { if (typeof reader.result === 'string') onChange({ ...data, company: { ...data.company, logoUrl: reader.result } }); };
      reader.readAsDataURL(file);
    }
  };
  ...
  const fetchBusinessReviews = async () => {
    ...
    try {
        ...
        const ai = new GoogleGenerativeAI(import.meta.env.VITE_GEMINI_API_KEY || '');
        const model = ai.getGenerativeModel({ model: "gemini-2.0-flash-exp" });
        const response = await model.generateContent(`Find the 3 most relevant and positive reviews for "${data.business.query}".
